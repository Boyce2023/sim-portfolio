...
## ⛔⛔ 每日必备: 12:00午间完整调仓(2026-08-28 Buwen固化令)

> 原话: "在12:00整开始做根据早上收盘价的A股完整调仓，以后这就是你每个交易日A股的必备活动"

**每个交易日12:00,根据早上收盘价(11:30)执行下方"调仓流程定义"的完整四步。** 不等提醒不等催——这是制度不是任务。
...
⛔**2026-09-07新增第⓪步: 先查待裁决台账**(`scripts/pending_verdicts.py list --due`)。任何在交易reason里承诺的复核/裁决节点, 下单当时就必须 `pending_verdicts.py add` 登记, 否则那句承诺等于没写。

| # | 环节 | 必须包含 | 我犯过的残缺形态 |
|---|---|---|---|
| ⓪ | **待裁决台账** | ⛔**四步之前先跑** `python3 scripts/pending_verdicts.py list --due`, 有到期项先处理完再往下走。结案用 `pending_verdicts.py done <id> <结论>` | 2026-09-07: 我08-24减半概伦时写明'保留一半等08-27中报裁决', 中报披露**11天后**才发现那次裁决从未执行。回扫trade_log发现类似承诺共55条, 全都只存在于reason的一段文本里, **没有任何机制会在到期日提醒我**。⛔漏做比做错更危险: 做错留下痕迹能复盘, 漏做是静默的 |
...
## ⛔ 取用链 — 动作前必跑(2026-09-08 Buwen定)

他的原话: **「学了很多东西, 脑子里也有很多东西, 但是在对应的时候不调用, 就感觉学了很多没用到点子上; 对于很多事, 要有一个一针见血的从头到尾的逻辑链」**

⛔病根不是知识不够(memory 100+文件/行为规则324条), 是**取用这一步不存在于流程里**, 全靠"我恰好想起来"。
09-08 实证: 锚点写在我自己发的信号里却没在交付前跑 / 学完交易规则还是把撤单当卖出 /
一整天给别处建门自己路径上一个没有 / 09-04撤回的数字09-08又当事实用。**每次都不是不知道, 是没去取。**

```bash
python3 scripts/preflight.py deliver   # 交付表格/数字前
python3 scripts/preflight.py buy 600111  # 买入前(自动跑历史公告回溯)
python3 scripts/preflight.py sell      # 卖出前
python3 scripts/preflight.py answer    # 回答事实问题前
```

已挂在路径上: 早盘 ping(buy+sell链) / b_auto_trade 执行前(buy链) / 收盘四步第①步(deliver链)。

## ⛔⛔ 建仓前强制门 — 历史公告回溯(2026-09-07定, 与 SABCT≥A- 同级)

**任何 A 股建仓前必须先跑历史公告回溯, 有 🔴 红旗就停下来读完原文再决定。**

```bash
cd ~/claude-projects/news-dashboard && python3 announce_lookup.py <ticker> 3
```

**为什么是硬门不是建议**: 2026-09-07 兴业银锡清仓实现 -19,389。银漫矿业占其利润约 80%,
7-26 井下事故致 1 死、7-28 停产公告——而我 8-26 建仓时这件事已经发生整整一个月,
持有的 12 天里从不知道真正风险在哪。**8-26 建仓前跑这一行就能看到。**

同日用它扫全部 8 只持仓, 又查出川能动力 08-25 计提 8,508 万(存货跌价 7,976 万＝锂库存减值),
与建仓 thesis「受益锂价弹性」直接矛盾, 当日减半。**一天之内这道门抓出两笔。**

**判读**:
- 🔴红旗(停产/事故/立案/处罚/诉讼/冻结/预亏/减值准备/退市/违规) → 必须读 PDF 原文
- 🟡黄旗 → 看性质
- ⚠️「自查报告/专项说明/审核报告/法律意见书/问询函回复」是程序性附件, 不是风险(main 已加 PROCEDURAL 词表优先降黄)
- ⛔ 查到 0 条**先怀疑查询本身**, 一家公司三个月零公告几乎不可能
- ⚠️ PDF 下到 0 字节时先怀疑限流或网络抖动并重试, **不要归因到请求头**。
  (09-07 我曾报「必须带 Referer」, 09-08 单变量复测证伪: 裸 curl / 只加 -L / 只加 UA /
  只加 Referer / 全加, 六种组合全部 251,636 字节完全一致, 裸请求 HTTP 200 零重定向。
  当时我一次改三个参数却归因给一个, 还套上当天反复在用的「故障降级成空值」模式——
  **一个自带说服力的解释会挡住真因**。详见 memory/feedback_single_variable_attribution)
...
## ⛔⛔ B策略专户(2026-09-03 Buwen令"持仓里3成送给b策略")

**规模**: 建户时 NAV 912.4万 × 30% = **273.72万**。单笔上限 91.24万,最多同时持3只。账本 `data/b_book.json`。

**⛔ 两本账物理隔离**: B的盈亏不计入基本面盘的选股能力评价,反之亦然。B买卖不看基本面,基本面买卖不看板。共用现金池但B用量上限=pool_size,超了不许开新仓。⛔B买过的标的不因此进基本面watchlist(防两套系统交叉污染,同ABCDEFG隔离铁律)。

**买入**: 当日触涨停价即以涨停价买。核心过滤(1001笔回测四期胜率79/80/80/84%): **前1日涨>3% 且 当日量比<1.5**。
**封单质量四道门**(2026-09-03新增,治可成交性): ①首封<10:00 ②炸板=0 ③封板资金/成交额≥3% ④所属行业当日涨停≥2只(非孤板)。**4/4满档 · 3/4半档 · ≤2/4不买**。
**卖出**: 次日开盘价卖,不留第二天。B没有thesis只有溢价。
**禁买**: ST/次新(N/C开头) · 流通<10亿或>300亿 · 当日换手>12% · 首封晚于14:00。

**每日闭环**: 收盘版调仓生成次日短名单 `data/b_watch_YYYYMMDD.json`(封单四门排序) → `scripts/b_board_alert.py`(launchd 09:27)开盘读它,临板/已封推送带⭐标记 → 盘中按四门决定满档/半档/不买 → 次日开盘卖。

**⛔时点限制**: B的买点是盘中触板,收盘后无法建仓。收盘时段只能划池+定名单,第一笔成交必在次一交易日。
