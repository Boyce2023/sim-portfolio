#!/usr/bin/env python3
"""Build 2021 A-stock daily price database using yfinance (proxy-friendly)."""
# /// script
...
def main():
    price_db = {
        "_meta": {"year": 2021, "source": "yfinance", "stock_count": len(STOCKS), "built_at": datetime.now().isoformat()},
        "stocks": {}
    }
    success, failed = 0, []
    ...
    for code, name in STOCKS.items():
        ...
        try:
            t = yf.Ticker(yft)
            df = t.history(start="2021-01-01", end="2022-01-01")
    ...
    out = "/Users/huaichuaibeimeng/claude-projects/sim-portfolio/research-notes/astock-backtest/2021/infra/price_db_2021.json"
    ...
    # Also extract trading calendar
    if price_db["stocks"]:
        sample = next(iter(price_db["stocks"].values()))
        cal = [r["date"] for r in sample["daily_prices"]]
        cal_path = "/Users/huaichuaibeimeng/claude-projects/sim-portfolio/research-notes/astock-backtest/2021/infra/trading_calendar_2021.json"
        with open(cal_path, "w") as f:
            json.dump({"year": 2021, "trading_days": cal, "count": len(cal)}, f, indent=1)
