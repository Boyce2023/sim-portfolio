...
## §5 卖出纪律（V7.0 — Thesis-Based Exit）

**核心原则：价值投资者只在thesis失效时卖出，不在价格下跌时卖出。**

| 信号 | 动作 | 优先级 |
|------|------|--------|
| Thesis证伪（D-type） | 全部卖出，不犹豫 | 唯一自动卖出触发 |
| 止损参考价触及 | **THESIS REVIEW** — 重新评估thesis。thesis没变→持有/加仓；thesis动摇→卖 | 审查，非自动卖 |
| 催化剂兑现 | 检查催化剂链：下一个<3个月→持有；无后续→减仓 | 审查 |
| **Thesis充分兑现** | **催化剂链已兑现 + 增长开始减速 + 市场在找下一个故事 → 卖出。不是"涨多了"，是剩余alpha<持有机会成本** | 审查 |
| 更好的机会 | 先确认新机会thesis成立，再换 | 低 |
| 90天无催化剂 | 标记审查，不自动卖出 | 低 |

**不卖的情况**：
- 股价跌了但thesis没变 → **持有甚至加仓**（这是价值投资的核心：便宜了买更多）
- 短期波动 → 无视
- 市场恐慌但公司基本面没变 → 持有，VIX飙升是买入窗口不是卖出信号
- 股价涨了很多但增长驱动力仍在持续+催化剂链完整 → 赢家要加不要卖（NVDA式：涨了5年thesis还在验证中）
- **⛔ ATR/机械止损触发 → 绝不自动卖出。** 止损价仅作参考（advisory），触发后进入thesis review流程

### §5.1 价值投资者回撤协议（Value Investor Drawdown Protocol）

**大跌 = 打折买入机会，不是逃跑信号。** 与A股短线系统完全不同。

| 回撤幅度 | 动作 | 逻辑 |
|----------|------|------|
| -5%~-10% | 正常波动，无视 | SPY年均回撤-14%，个股-10%是常态 |
| -10%~-20% | **Thesis Review** — thesis intact → 加仓窗口 | 价值投资者在别人恐惧时贪婪 |
| -20%~-30% | **Deep Review** — 重新验证供给侧+催化剂 → thesis intact则重仓加 | 2022式回调，NVDA -66%后涨10倍 |
| >-30% | **Emergency Review** — 是否有结构性变化？→ 没有就拿住 | 价值投资者最大的利润来源 |

**⛔ 绝不做的事：**
- 因为价格下跌而恐慌卖出 → 这是momentum trader的行为
- 设置自动止损单（代码中ATR stop = advisory only）
- 在VIX>30时清仓 → VIX>30恰恰是加仓时机
- 因为"已经跌了很多"就认为还会跌 → 价格不预测价格，基本面预测价格

**SOXL教训（2026-06）：** 在回调中清仓1036股SOXL@$211，锁定$66K亏损，随后同价位买回。价值投资者在conviction标的大跌时应该加仓，不是割肉。机械止损+circuit breaker+VIX阻断 = 三重系统性错误，V7.0已全部修正。

### §5.2 杠杆ETF特殊规则

杠杆ETF（SOXL/TQQQ/NVDL等，完整清单见config.py `LEVERAGED_ETF_NO_STOP`）：
- **无机械止损** — 3x ETF的ATR是标的3倍，机械止损在正常波动中必然触发
- **只看thesis退出** — 如果看多NVDA/半导体，SOXL的价格波动不改变thesis
- **深度亏损仅触发thesis review** — 不自动卖出，重新验证底层逻辑即可

### §5.3 与A股系统的区别

| 维度 | 美股（本策略） | A股（strategy_astock.md） |
|------|---------------|-------------------------|
| 退出哲学 | Thesis-based（只在thesis失效时卖） | 机械规则（止损/尾随止盈/信号驱动） |
| 止损 | Advisory参考，不自动执行 | 硬止损，自动触发 |
| 回撤处理 | 大跌=加仓机会 | 大跌=止损保护本金 |
| 持有期限 | 无上限（thesis有效就拿） | 短线为主，14天无催化剂则审查 |
| Circuit breaker | 审查触发，不自动卖 | 硬性断路器 |
