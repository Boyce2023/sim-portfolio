...
const VERDICT={type:'object',properties:{ticker:{type:'string',description:'⛔必填:本次裁决标的的6位代码。2026-09-07加——此前VERDICT不含ticker,裁决与标的靠数组下标 vs.map((v,i)=>({...cands[i]})) 对齐,一旦并行返回顺序错位就张冠李戴且不会报错(与雄帝科技错配同形)。agent自报后与cands[i].ticker交叉校验,不一致立刻报出来。'},name:{type:'string',description:'标的简称,与ticker一并用于交叉校验'},decision:{type:'string',enum:['probe','watch','reject','hold'],description:'⛔二维裁决:probe=基本面好+主升中(现价进)/watch=基本面好但末段见顶(等回踩,不否定基本面)/reject=基本面差(概念蹭/暴雷/估值无边际,与涨跌无关)/hold=已持仓'},fundamental:{type:'string',description:'基本面轴(定值不值得买):好/差+依据(Edge真假/份额/概念蹭/暴雷/估值安全边际)'},trend:{type:'string',description:'量价轴(定时机):主升中/末段见顶/下跌——看量价结构非涨幅'},sabct:{type:'string'},size_now:{type:'string',description:'现价建多少仓(基本面好+主升中才填实)'},stop:{type:'string'},catalyst_date:{type:'string'},watch_expiry:{type:'string',description:'⛔watch必填三件套(T16不许挂空等回调,江丰踏空+77.7%/北方华创+57%教训):①回踩买点位②失效期5-8交易日③N日未回踩动作(链趋势重启/放量新高→按趋势追;走坏→放弃)。probe/reject可留空'},one_line:{type:'string'}},required:['ticker','decision','fundamental','trend','sabct','size_now','one_line']}
const seen=new Set()
const step1trees=[]
...
const chains=await pipelineWaves(TREES,
  ...
  `⛔⛔涨停判定铁律(2026-09-07实证修正): 涨停幅度按板块分——**主板(60/00开头)=10%, 创业板(30)/科创板(68)=20%, 北交所(43/83/87/92)=30%**。⛔严禁把创业板/科创板涨10%当涨停! 当日实证: agent报'全市场涨停148只', 官方涨停池实为93只, 差额全是把中际旭创+10.38%/太辰光+14.5%/罗博特科+10.1%/德福科技+16.97%这些20cm股当成了涨停。后果是把'普涨式跟风(93只/炸板率61%)'误读成'情绪极热(148只)', 直接影响仓位系数。**涨停只数一律以 ak.stock_zt_pool_em(date=...) 官方接口为准, 不要自己按涨幅数。**\n`+
  ...
  `⛔⛔返回JSON必须回填 ticker 与 name 两个字段, 填你**实际裁决的那只**的6位代码与简称(不是照抄题面——照抄就失去校验意义)。主脑会拿它与派发时的标的交叉校验, 对不上即判错配作废。2026-09-07加: 此前裁决与标的靠数组下标对齐, 并行返回顺序一错就张冠李戴且无人知晓。\n`+
  ...
  {model:'claude-sonnet-5',schema:VERDICT,label:c.name,phase:'并行主体(宏观+18树流水线深扫+持仓)'}))).then(vs=>({tree:tr,verdicts:vs.map((v,i)=>{const c=cands[i]; if(v&&v.ticker&&norm(v.ticker)!==norm(c.ticker)){log(`⛔⛔裁决错配: agent自报${v.ticker}(${v.name||'?'}) 但按下标应为${c.ticker}(${c.name||'?'}) —— 本条作废,不进候选`);return {...c,verdict:null,_mismatch:{claimed:v.ticker,expected:c.ticker}}}return {...c,verdict:v}}).filter(x=>x.verdict&&!HELD.includes(norm(x.ticker)))}))
  })
