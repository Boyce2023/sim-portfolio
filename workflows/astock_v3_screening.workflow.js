...
const chains=await pipeline(TREES,
  ...
  `⛔watch三道闸(T16,实盘教训江丰等回调踏空+77.7%/北方华创+57%,回调最深仅-0.3%):①watch必须真末段(有硬见顶信号:天量巨阴/放量滞涨/破位),⛔所在链若正主升早段(第二波重启/链内涨停潮),该票的回调很可能是新主升起点,慎判watch宜判probe;②A级+供给侧物理约束(管制/矿/认证)的票原则上不判watch(涨停=建仓信号);③watch必填watch_expiry三件套(回踩位+失效期5-8日+未触发动作),不许挂空等回调;④⛔见顶链回踩=接刀(07-06实证:钨链06-22天量见顶后,07-02"缩量回踩企稳"触发probe→中钨-13.7%止损/章源-10%,见顶后的回踩是下跌中继不是买点):该标的所在链若已出现链级见顶信号(天量巨阴/放量滞涨/龙头破位),watch的回踩位触发**作废**,只保留"放量收复前高/启动平台上沿"触发——V型收复才是真重启。\n`+
