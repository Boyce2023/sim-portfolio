...
const results = await pipeline(
  stocks,
  (c) => agent(`${D(c)}\n决策维度①供给侧Edge: 别人给不了什么? 物理/制度壁垒? 谁有定价权? 优势维持多久? 在主线传导链里它是哪一层(大硬件/小硬件材料/矿)、这层炒透了没?`, { label: `Edge:${c.name}`, model:'claude-sonnet-5',phase: '5维深扫' }),
  (edge, c) => agent(`${D(c)}\n决策维度②Kill Shot: 什么能一票否决? 专搜负面(份额假/估值透支/催化证伪/周期顶/概念蹭非真受益)。\n供给侧参考: ${String(edge).slice(0, 600)}`, { label: `Kill:${c.name}`, model:'claude-sonnet-5',phase: '5维深扫' }),
  (kill, c) => agent(`${D(c)}\n决策维度③定价检验: 现价+PEG(G标来源)+前瞻PE(26E/27E,爬坡股用季度斜率)。已涨多少? price in到哪? 在主线里是已炒透的层还是没炒到的埋伏层?`, { label: `Price:${c.name}`, model:'claude-sonnet-5',phase: '5维深扫' }),
  (price, c) => agent(`${D(c)}\n决策维度④催化剂锁定: 何时能证明判断对? 具体事件+日期? 不兑现怎么办? 主线传导位置(启动/主升早/主升中/台阶/尾声)?`, { label: `Cat:${c.name}`, model:'claude-sonnet-5',phase: '5维深扫' }),
  (cat, c) => agent(`${D(c)}\n决策维度⑤现价裁决(综合前4维): ⛔只回答"以现价值不值得押一注"——严禁"等回调到XX"(那是预测价格,无信息优势必失效,详见memory/feedback_no_wait_pullback.md)。\n决策第一问: 现价这笔风险收益值不值得押注?\n- 没炒透的非共识埋伏点→probe现价小仓5-8%占位(它没涨,等=等它被发现后涨上去=踏空)\n- 已炒透共识(涨停板/抛物线顶)→reject,不是等回调(你不知道它会不会回调,等回调是假装你知道)\n- thesis软→reject\nSABCT(A-最低门槛)。执行卡片: 现价建多少仓(size_now,禁写"等回调")/止损-12%/出场(催化剂过+什么信号)/催化剂日期。结论先行。`, { schema: VERDICT_SCHEMA, label: `裁决:${c.name}`, model:'claude-sonnet-5',phase: '5维深扫' })
)
