#!/usr/bin/env python3
"""样本外验证: 2026年7-8月数据补齐(2026-09-07 Buwen要求更新到8月底)
...
codes=[x[0] for x in _sq.connect(f'{BASE}/univ202606.db').execute("select distinct code from k")]
...
for i,c in enumerate(todo):
    try:
        ...
        rs=bs.query_history_k_data_plus(c,"date,code,open,high,low,close,preclose,volume,amount,turn,isST",
            start_date="2026-06-20",end_date="2026-09-05",frequency='d',adjustflag='3')
        rows=[]
        while rs.next():
            d=rs.get_row_data()
