...
con.commit()
signal.alarm(60); rs=bs.query_all_stock(day='2026-03-27'); signal.alarm(0)
codes=[]
while rs.next():
    c=rs.get_row_data()[0]
...
for i,c in enumerate(todo):
    try:
        ...
        rs=bs.query_history_k_data_plus(c,"date,code,open,high,low,close,preclose,volume,amount,turn,isST",
            start_date="2026-01-15",end_date="2026-04-20",frequency='d',adjustflag='3')
        rows=[]
        while rs.next():
            d=rs.get_row_data()
