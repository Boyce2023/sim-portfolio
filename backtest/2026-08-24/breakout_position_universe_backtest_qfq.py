...
Data source: akshare ak.stock_zh_a_daily (Sina, unadjusted adjust="qfq") -- D12-compliant
...
def _fetch_raw(code):
    df = ak.stock_zh_a_daily(symbol=prefix(code), start_date=FETCH_START, end_date=FETCH_END, adjust="qfq")
...
def main():
    ...
    json.dump({"summary": fetch_log, "failures_sample": fail_list[:60], "n_failures_total": len(fail_list)},
              open(f"{OUTDIR}/fetch_log_qfq.json", "w"), ensure_ascii=False, indent=2)
    ...
    with open(f"{OUTDIR}/daylevel_records_qfq.csv", "w", newline="") as f:
    ...
    json.dump({"meta": meta, "buckets": result}, open(f"{OUTDIR}/bucket_stats_qfq.json", "w"),
               ensure_ascii=False, indent=2)

    # flat CSV
    with open(f"{OUTDIR}/bucket_stats_qfq.csv", "w", newline="") as f:
        w = csv.writer(f)
