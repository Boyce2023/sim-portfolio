"""⛔【2026-09-04 二次修正】本文件用 engine_b2_fix(已修涨停判定)重跑。
原 vr_test.py 用的是未修复的 engine_b2, 结论同样建立在只识别约1/4板的有偏样本上——
us session自查时发现 build_limitup.py 与 engine.py 仍有多处0.005容差, 我顺藤查到 engine_b2 也没换,
而 engine_b2_fix.py 早就写好却从未被引用。⛔本文件的数字取代 vr_test.py 的一切结论。

原说明: 检验: B核心规则里的'量比<1.5'是事后变量吗? 换成买入前可知的口径后edge还在吗?
...
from engine_b2_fix import build
...
json.dump(R,open('/tmp/vr_rows_fixed.json','w'))
