...
# ⛔彻底根治东财被代理软件劫持(2026-07-01): 用户机器有代理软件(Clash/Surge类)把eastmoney的DNS
# 劫持到198.18.2.75虚拟IP+代理规则挡截,走代理必ProxyError;设NO_PROXY绕代理直连东财(实测返回200)。
# 必须在import requests/akshare之前设。此后所有走本模块的东财接口不再被挡,不用绕腾讯兜底。
import os as _os
