...
def decide_buy(sv):
    ...
    # ══════════════════════════════════════════════════════════════════
    # ② 现在买(量价轴) —— 2026-08-06 第2轮红队证伪后定版
    #
    # ⛔⛔ 第1轮的"位置门双峰/深跌区最优"已被第2轮全盘推翻,是幸存者偏差的产物。不要再复活它。
    #   第1轮样本: 30只"6大科技子行业各挑5只"——这30只是2026-08用今天的眼光挑的当前代表股。
    #   第2轮复现: 335只全市场随机抽样(种子20260806,剔ST/北交所/次新,40638个信号点,Sina日线220根)
    #     区间          第1轮(手选30只)      第2轮(无偏335只)   20日
    #     A[-3,+8]      +8.93% / 61.8%      -0.55% / 41.6%
    #     B[-8,-3]      +8.28% / 58.6%      -2.01% / 35.7%
    #     C[-15,-8]     +4.77% / 51.6%      -2.66% / 34.3%
    #     D[-25,-15]   +12.43% / 67.3%      -2.51% / 35.9%
    #     E[-40,-25]   +13.92% / 65.3%      -3.95% / 34.6%
    #     F[<-40]      (n=24不可用)         -7.31% / 29.4%
    #   A vs D t=+6.57 p<0.0001 | A vs E t=+8.63 p<0.0001 —— 方向完全相反, 胜率塌30个百分点。
    #   ★幸存者偏差的直接证据(按2026全年涨跌中位数-15.81%分赢家/输家组):
    #     D区20日 赢家组+1.93%(n=3982) vs 输家组-5.57%(n=5783), t=+21.11 p<0.0001
    #     E区20日 赢家组-0.13% vs 输家组-6.94%, t=+10.60 p<0.0001
    #     即"深跌区好"完全取决于该股最终是不是赢家, 而"最终是赢家"正是第1轮选样时用今天的眼光锁定的条件。
    #     且即使在赢家组内部 D(+1.93%)/E(-0.13%) 也没跑赢 A(+2.76%) —— 相对排序在最有利子样本里也不成立。
    #   → 正确形状 = 单调动量/趋势延续: 越贴近25日高越好, 越远越差。C区不是U型底, 是下坡途中。
    #
    # ⛔ 通道3"深跌价值"整条删除(连同其中的 0<PE<=30 硬门):
    #   ①它建立在上面被证伪的发现之上;
    #   ②PE硬门违反本项目 D8估值宪法(PEG唯一, 静态/前瞻PE禁止单独用于估值判断)
    #     与 memory/feedback_trailing_pe_ramp(PCB事故: 爬坡股trailing PE天然虚高, 用它挡龙头
    #     =澜起+40%/长电+47%踏空)。用户2026-08-06当场指出"我说了不看PE"。
    #   ③第2轮实测: 所谓"PE<15胜率62.2% vs PE>100仅40.9%(4个标准差)"是 backsolve-PE 前视偏差的
    #     算法产物, 用真实历史PE重测后效果缩水到 +2.8pp(50.9% vs 48.1%); 另一agent因拿不到历史PE
    #     面板直接返回 INSUFFICIENT_DATA。该结论从未通过样本外验证。
    #
    # ⭐ 那"位置门死锁"(高位说等回调/跌了说位置不合格)怎么破? 不在深跌端破, 在"等回调"端破:
    #   ①memory/feedback_no_wait_pullback: 等回调对没信息优势的人系统性失效, 且回调真来了位置反而更差
    #     —— 这正是死锁的成因, 解药是当初就别说"等回调", 不是事后放行深跌。
    #   ②第2轮大盘股豁免(见LARGE_CAP): 大盘股当日涨5-8%后 fwd20超额+5.08%(t=8.80), >11%非涨停+8.72%
    #     (t=5.13) —— 大盘龙头"涨太多"根本不是等的理由, 这才是08-06砍掉工业富联/胜宏的真错因。
    #   ③深跌股要买, 必须叠加独立的突破确认(重新进入A区), 不能靠跌幅深度本身当扳机。
    # ══════════════════════════════════════════════════════════════════
    ...
    vp = tr.get('量价结构')
    mcap = tr.get('市值亿') or sv.get('market_cap') or 0
    big = mcap >= LARGE_CAP

    in_break = -3.0 <= brk <= 8.0          # A区: 无偏样本里最好的区间
    in_near  = -8.0 <= brk < -3.0          # B区: 次之
    # C及以下(brk<-8): 不作为独立买入触发条件。要买必须重新突破回A区。
    ...
    # ⭐普跌门(2026-08-06第2轮定版新增, 唯一被四种切法一致验证的regime用法)
    # 证据(breakout_regime, 四种切法方向全部一致为负):
    #   同日口径 5/10/20日超额 -1.71/-1.54/-2.97pp | 滞后1日 -1.31/-1.66/-3.64pp
    #   ★最严格的T+1开盘可执行口径 -1.16/-1.59/-2.90pp (可执行率92.6%)
    #   换成"站上60日线占比"这个真状态变量(1日自相关0.975) -0.99/-1.67/-5.27pp
    # ⛔第1轮引用的 t=7.93/6.86/8.96 全部作废(伪重复:把同一天几十只信号股当独立样本)。
    #   按信号日聚类校正后真实强度 |t|≈2.1~3.0, 独立交易日仅44~65个, 循环移位 p=0.04~0.18(边缘显著)。
    #   置信度=中, 不是近乎确定。今后禁止再引用那三个t值。
    # 为何这里能用涨家占比(而sizing不能): 这是"逐信号即时0/1门",每天重判,不需要regime持久;
    #   当日涨家占比1日自相关-0.067(1.4天翻转)对多日持仓的仓位乘数是致命的,对当日门不是问题。
    if reg.get('water_level') == '普跌' and not a_level:
        return dict(action='watch', reason='普跌日(当日涨家占比≤35%)暂停新开A-级突破仓→次日重判不跨日锁定;A+/A级不受此门限制(T16防踏空)')
    if limit_up:
        # ⚠️第2轮实测"次日回踩不破前低才买"这条子规则几乎不筛(28476个涨停日通过25431个=89.3%),
        #   且入场后持3日+0.33%(t=1.95临界)/持5日-0.19%/持10日-0.37%, 胜率全程<50% —— 无稳健edge。
        #   涨停本身的动量是真的但1-10日内衰竭(fwd1+1.77% t=22.45 → fwd20-0.25%不显著), 且涨停价买不到。
        return dict(action='打板/次日回踩', reason='涨停封板→现价追=接盘;⚠️"次日回踩不破前低"这条实测89.3%都能过=几乎不筛且胜率<50%,别当它是安全垫,要追就承认是追动量(fwd1+1.77%但10日内衰竭)')
    if blow_off:
        return dict(action='watch', reason=f'⛔天量见顶(60日量比{vr60:.1f}≥{VOL_BLOWOFF}):实测vr60基准下与未来收益显著负相关(t=-8.94~-12.72),是派发不是突破→等量能回落')
    if is_top:
        return dict(action='watch', reason='末段放量滞涨→等回踩(必设失效期,链重启/放量新高则转追,别死等)')
    if ext:
        if a_level:
            size = round(cap*mult*(1.0 if has_vol else 0.5), 3)
            t_ = '大力' if has_vol else '小仓'
            return dict(action=f'probe/买-{t_}', size_pct=size, tier=t_, stop_type=stop,
                        reason=f'A级豁免"冲太高":距前高{brk:.1f}%>8%但A级+供给侧物理约束(T16涨停豁免,防江丰+77.7%/北方华创+57%式踏空)→{size*100:.0f}%')
        # ⚠️大盘股豁免已撤回(前视偏差,见LARGE_CAP注释),此处只记读数不放行
        _big_note = f';[影子]市值{mcap:.0f}亿≥{LARGE_CAP:.0f}亿,若大盘豁免成立本应放行——记台账待point-in-time市值验证' if mcap >= LARGE_CAP else ''
        return dict(action='watch', reason=f'非A级且距前高{brk:.1f}%>8%冲太高→等回踩(防追高全市场证据:5-8%桶fwd20超额-0.30~-0.60pp/|t|2.20~4.02,2025+2026两年同向){_big_note}')

    if in_break:
        size = round(cap*mult*(1.0 if has_vol else 0.5), 3)
        t_ = '大力' if has_vol else '小仓'
        return dict(action=f'probe/买-{t_}', size_pct=size, tier=t_, stop_type=stop,
                    reason=f'突破档:值得买(SABCT {sabct})×距前高{brk:.1f}%∈[-3,+8](无偏样本里最好的区间,A vs D t=+6.57/A vs E t=+8.63均p<0.0001)×{"有量" if has_vol else "无量(半档)"}(量比{vr:.1f})→{size*100:.0f}%;止损={stop}')
    if in_near:
        size = round(cap*mult*0.5, 3)
        return dict(action='probe/买-小仓', size_pct=size, tier='小仓', stop_type=stop,
                    reason=f'近突破档:值得买(SABCT {sabct})×距前高{brk:.1f}%∈[-8,-3)(次于A区)→半档{size*100:.0f}%;重新突破回A区可加至大力档;止损={stop}')
    return dict(action='watch',
                reason=f'距前高{brk:.1f}%<-8%→⛔深跌本身不是买入理由(无偏335只样本:D区20日-2.51%/胜率35.9%,E区-3.95%/34.6%,F区-7.31%/29.4%,单调恶化;第1轮"深跌区胜率67%"已证实是幸存者偏差)。要买必须等它重新突破回[-3,+8]区间,靠跌幅深度抄底在全市场样本上是负期望')
