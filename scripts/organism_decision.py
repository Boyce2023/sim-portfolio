...
# ⭐大盘股豁免线(2026-08-06第2轮回测新增, 唯一被无偏样本证实的"追强"证据):
...
def decide_buy(sv):
    ...
    if ext:
        if big or a_level:
            # 大盘股/A级: 不因"冲太高"拦。大盘股有硬证据(涨幅越大fwd20超额越高); A级沿用T16豁免。
            size = round(cap*mult*(1.0 if has_vol else 0.5), 3)
            t_ = '大力' if has_vol else '小仓'
            # ⚠️别把%格式化嵌进f-string表达式(裸%会被当格式符,已踩两次TypeError),先算好字符串再插值
            why = ('市值{:.0f}亿≥{:.0f}亿(大盘股当日涨5-8%后fwd20超额+5.08%/t=8.80,'
                   '>11%非涨停+8.72%/t=5.13,涨得多反而更强)').format(mcap, LARGE_CAP) if big \
                  else 'A级+供给侧物理约束(T16涨停豁免)'
            return dict(action=f'probe/买-{t_}', size_pct=size, tier=t_, stop_type=stop,
                        reason=f'{"大盘股" if big else "A级"}豁免"冲太高":距前高{brk:.1f}%>8%但{why}→{size*100:.0f}%;⛔这条就是08-06砍掉工业富联/胜宏的错因')
        return dict(action='watch', reason=f'中小盘(市值{mcap:.0f}亿<500亿)且非A级,距前高{brk:.1f}%>8%冲太高→等回踩(中小盘防追高有硬证据:5-8%桶fwd20超额-0.83pp/t=-3.31)')
