...
# ⛔ 量比门解除对regime的依赖(2026-08-06):
#   病史: 07-29设1.5 → 08-03发现缩圈市场23个A-候选无一达标(最高1.49)降到1.0 → 08-06转普涨又升回1.5又全挡掉。
#   根因: 阈值挂在regime上,而regime判定已被回测证伪(自相关-0.09/循环移位p=0.18~0.77/盘中vs收盘一致率49.3%)。
#   拿一个噪音信号去调另一个门的阈值 = 噪音的平方。
#   现改为恒定1.2(不分regime): 取"温和放量"而非"爆量",避免只在极端行情才有候选。
#   ⚠️1.2这个数暂无强回测支撑(第1轮迭代的vol_gate专项正在验证),定版前按"宁松勿紧"处理——
#     漏买(门太严)在过去两月的实证代价是-13pp机会成本,买错有-12%止损兜底,两者不对称。
VOL_GATE = {'普涨': 1.2, '缩圈': 1.2, '普跌': 1.2}
