...
# 判断层归research(2026-09-03 Buwen令): 材料落盘后投research inbox一条通知
PREP=$(ls -t /tmp/us_jump_prep_*.md 2>/dev/null | head -1)
if [ -n "$PREP" ]; then N=$(grep -c '^## ' "$PREP"); printf '{"ts":"%s","from":"us-morning-cron","kind":"task","instruction":"[跳涨判断材料已落盘] %s 共%s只昨夜≥10%%跳涨(含公司介绍/市值/前1月/基础率格子/新闻), 判断层归research: 请接共识层逐只判断并回Buwen; 基础率note research-notes/us-database/2026-09-03_跳涨后走势_基础率.md"}\n' "$(date '+%Y-%m-%dT%H:%M:%S%z')" "$PREP" "$N" >> "$HOME/.claude/session-inbox/research.jsonl"; fi
