...
## §0.5 信息检索优先级（两市场共用铁律，见CLAUDE.md §5 R2）

用户问"新闻/市场/最新/动态/发生什么了" → **必须先读内部数据源，不够再WebSearch补搜**

| 用户说的 | 第一步 | 第二步（仅当第一步不够时） |
|---------|-------|------------------------|
| 新闻/最新/大事/发生了什么 | 读 `latest_news.json`（筛breaking+important） | WebSearch补搜缺失维度 |
| 对面市场/A股那边 | 读 `cross_intel_brief.json` | WebSearch补搜 |
| 持仓/portfolio | 读 `portfolio_state.json` | 无需外搜 |
| 价格 | `yf price TICKER` | 无需外搜 |
| Regime/市场状态 | `regime_detection.py` | 无需外搜 |

**绝不跳过内部数据直接外搜。** 补搜时主动说明："系统新闻没覆盖X方向，我补搜。"
